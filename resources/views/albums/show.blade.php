...
@section('content')
 ...
 @if(count($album->photos) > 0)<!--here we are checking if there are any photos... -->
    ...
    ?>
    <div id="photos">
      <div class="row text-center"><!--and this is our first row with the photos -->
        @foreach($album->photos as $photo)
          @if($i == $colcount)<!--if there is only 1 photo, do this here -->
             <div class='medium-4 columns end'>
               <a href="/photos/{{$photo->id}}"><!--link to the actual photo -->
                  <img class="thumbnail" src="/storage/photos/{{$photo->album_id}}/{{$photo->photo}}" alt="{{$photo->title}}"><!--link to the image -->
                </a>
               <br>
               <h4>{{$photo->title}}</h4>
